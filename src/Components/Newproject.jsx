...
function Newproject({ setPopup }) {
    ...
    const handleSave = (e) => 
    {
        if (e) e.preventDefault() // Prevent full page reload on submit)

        async function postData() {
            try {
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/newprojects`,
                    {
            ...
            <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#2DD4BF]/20 bg-[#061719]/95 shadow-2xl shadow-[#14B8A6]/10 backdrop-blur-2xl">
                ...
                <form onSubmit={handleSave}>
                    <div className="space-y-4 p-6">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <select
                                    value={category}
                                    className="h-10 w-full rounded-xl border border-[#2DD4BF]/15 bg-[#061719] px-3 text-sm font-medium text-gray-200 outline-none focus:border-[#14B8A6] focus:ring-1 focus:ring-[#14B8A6]/50 transition-all"
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input
                                    type="date"
                                    value={deadline}
                                    className="h-10 w-full rounded-xl border border-[#2DD4BF]/15 bg-white/[0.03] px-3.5 text-sm font-medium text-white [color-scheme:dark] outline-none focus:border-[#14B8A6] focus:bg-white/[0.06] focus:ring-1 focus:ring-[#14B8A6]/50 transition-all"
                    ...
                    <div className="flex items-center justify-end gap-3 border-t border-white/10 bg-white/[0.02] px-6 py-4">
                        <button
                            type="button"
                            className="h-10 rounded-xl border border-white/10 bg-white/5 px-5 text-xs font-semibold text-gray-300 hover:bg-white/10 hover:text-white transition-colors"
                            onClick={() => setPopup(false)}
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className="h-10 rounded-xl bg-gradient-to-r from-[#14B8A6] to-[#10B981] px-5 text-xs font-bold text-[#061719] shadow-md shadow-[#14B8A6]/20 hover:bg-[#2DD4BF] transition-all"
                        >
                            Save to MongoDB
                        </button>
                    </div>
